...
const EcoStorage = {
  ...
  /**
   * Safely parses a JSON string from localStorage, returning fallback on error.
  ...
  _readJSON(key, fallback) {
    try {
      ...
      console.warn(`EcoStorage: failed to parse ${key}`, error);
      return fallback;
    }
  },
   ...
   */
  getProfile() {
    return this._readJSON(this.KEYS.PROFILE, null);
  },
   ...
   */
  getLogs() {
    return this._readJSON(this.KEYS.LOGS, []);
  },
  ...
  updateLog(id, updates) {
    ...
    logs[index] = Object.assign({}, logs[index], updates, { id });
    localStorage.setItem(this.KEYS.LOGS, JSON.stringify(logs));
    return true;
  },
   ...
   */
  deleteLog(id) {
    const filtered = this.getLogs().filter(entry => entry.id !== id);
   ...
   */
  getLogsByDate(date) {
    return this.getLogs().filter(entry => entry.date === date);
  },
  ...
  getLogsByCategory(category) {
    return this.getLogs().filter(entry => entry.category === category);
  },
   ...
   */
  getTipsProgress() {
    return this._readJSON(this.KEYS.TIPS, { completedTips: [], dismissedTips: [], lastUpdated: '' });
  },
  ...
  validateImportData(data) {
    ...
    if (!data.profile.quizAnswers || typeof data.profile.quizAnswers !== 'object') return false;
    if (!Array.isArray(data.logs)) return false;
    if (!data.tips || typeof data.tips !== 'object') return false;

    const required = ['transport', 'food', 'energy', 'shopping'];
    return required.every(key => key in data.profile.quizAnswers);
  },
  ...
  getStreak() {
    ...
    const datesWithLogs = new Set(logs.map(entry => entry.date));
    let streak = 0;
