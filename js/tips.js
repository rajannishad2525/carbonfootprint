...
const Tips = {
  ...
  render() {
    ...
    const heroRingHTML = EcoData.buildSVGRing({
      radius: 38, strokeWidth: 7, ratio: ratio, stroke: 'var(--color-white)',
    ...
    main.innerHTML = '<div class="screen-tips" aria-label="Personalized eco tips">'
      ...
      + Tips._buildHeroHTML(heroRingHTML, completedCount, rankedTips.length, totalTips)
      + Tips._buildFilterBar()
  ...
  _buildFilterBar() {
    ...
    return '<div class="tips-filter flex gap-8 mt-16" role="tablist" aria-label="Filter tips by category">'
      + filterBtns
      + '</div>';
  },
  ...
  getRankedTips(profile) {
    ...
    const scored = remaining.map(function(tip) {
      ...
      const impactWeight = Tips.IMPACT_WEIGHTS[tip.impact] || 1;
